            ...
            <Paper square={true} elevation={0} sx={{width: "100%", m: "2%"}} component={Stack} direction="row">
                <Paper elevation={0} sx={{width: "20%", paddingRight: "2%"}} component={Stack} direction="column" justifyContent="center">
                    <Typography variant='h7'>
                        {props.whatDoesSearchBoxDo}
                    </Typography>
                </Paper>
                <Paper square={true} elevation={0} sx={{width: "50%", border: 1}}>
                    <InputBase onKeyDown={(e) => {if(e.key === 'Enter'){props.routeChange()}}} 
                    value={props.searchBarValue} onChange={(e) => {props.setSearchBarValue(e.target.value)}} 
                    fullWidth={true} sx={{padding: "8px", fontSize: "1.0rem"}}></InputBase>
                </Paper>
