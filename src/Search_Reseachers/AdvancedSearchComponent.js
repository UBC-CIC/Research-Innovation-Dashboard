...
export default function Advanced_Search(props){
    ...
    let {SearchForWhat, AllWords, ExactPhrase, AnyWords, NoneOfTheseWords, Department, Faculty, yearFrom, yearTo, Journal} = useParams();
    ...
    const searchResearchersQuery = async () => {
        const researcherSearchResult = await API.graphql({
          ...
          variables: {includeAllTheseWords: AllWords,
    ...
    const searchPublicationsQuery = async () => {
        console.log(fromDate.slice(0,4));
        ...
        console.log(publicationsSearchResult.data.advancedSearchPublications);
    ...
    }, [SearchForWhat, AllWords, ExactPhrase, AnyWords, NoneOfTheseWords, Department, Faculty, yearFrom, yearTo, Journal, searchClicked]);
    ...
    return(
        <div>
            ...
            <AdvancedSearchRow routeChange={routeChange} whatDoesSearchBoxDo={"Include All These Words:"} howToDoSearchBox={"Type in the important words with spaces between them."}
            searchBarValue={includeAllTheseWordsSearchBarValue} setSearchBarValue={setIncludeAllTheseWordsSearchBarValue}/>
            <AdvancedSearchRow routeChange={routeChange} whatDoesSearchBoxDo={"Include These Exact Phrases: "} howToDoSearchBox={"Put exact phrases in quotes. For example: \"Singapore Economy\" "}
            searchBarValue={exactPhraseSearchBarValue} setSearchBarValue={setExactPhraseSearchBarValue}/>
            <AdvancedSearchRow routeChange={routeChange} whatDoesSearchBoxDo={"Include Any Of These Words"} howToDoSearchBox={"Put in words that you want to search for"}
            searchBarValue={anyWordsSearchBarValue} setSearchBarValue={setAnyWordsSearchBarValue}/>
            <AdvancedSearchRow routeChange={routeChange} whatDoesSearchBoxDo={"Do Not Include Any Of These Words"} howToDoSearchBox={"Put a minus sign infront of words you do not want to include. For example: -Economics -Asia"}
            searchBarValue={noneOfTheseWordsSearchBarValue} setSearchBarValue={setNoneOfTheseWordsSearchBarValue}/>
            
            
            <Grid container>
            <Paper square={true} elevation={0} sx={{width: "100%", pl: "2%", pr: "2%", borderTop: 1, borderColor: "rgba(0, 0, 0, 0.12)"}} component={Stack} direction="row">
                    <Typography variant='h6' sx={{m: "2%", ml: "0%"}}>
                        Narrow Search Results By:
                    </Typography>
            </Paper>
            <AutoCompleteDropDown value={filterDepartmentValue} setValue={setFilterDepartmentValue} title={"Filter Researchers By Department"} DropDownArray={allDepartments} />
            <AutoCompleteDropDown value={filterFacultyValue} setValue={setFilterFacultyValue} title={"Filter Researchers By Faculty"} DropDownArray={allFaculty} />
            <Paper square={true} elevation={0} sx={{width: "100%", ml: "2%", mr: "2%"}} component={Stack} direction="row">
                <Paper elevation={0} sx={{width: "20%", paddingRight: "2%"}} component={Stack} direction="column" justifyContent="center">
                </Paper>
                <Paper square={true} elevation={0} sx={{width: "24%", border: 0, mr: "1%"}} component={Stack} direction="row">
                    FROM
                </Paper>
                <Paper square={true} elevation={0} sx={{width: "24%", border: 0, ml: "1%"}} component={Stack} direction="row">
                    TO
                </Paper>
                <Paper elevation={0} sx={{width: "30%", paddingLeft: "2%"}} component={Stack} direction="column" justifyContent="center">
                </Paper>
            </Paper>
            <Paper square={true} elevation={0} sx={{width: "100%", m: "2%", mt: "0%"}} component={Stack} direction="row">
                <Paper elevation={0} sx={{width: "20%", paddingRight: "2%"}} component={Stack} direction="column" justifyContent="center">
                    <Typography variant='h7'>
                        Filter Publications By Year:
                    </Typography>
                </Paper>
                <Paper square={true} elevation={0} sx={{width: "24%", border: 1, mr: "1%"}} component={Stack} direction="row">
                    <InputBase value={fromDate} type='month' onChange={(event)=>{ setFromDate(event.target.value)}} fullWidth={true} sx={{padding: "8px", fontSize: "1.0rem"}}></InputBase>
                </Paper>
                <Paper square={true} elevation={0} sx={{width: "24%", border: 1, ml: "1%"}} component={Stack} direction="row">
                    <InputBase value={toDate} type='month' onChange={(event)=>{ setToDate(event.target.value)}} fullWidth={true} sx={{padding: "8px", fontSize: "1.0rem"}}></InputBase>
                </Paper>
                <Paper elevation={0} sx={{width: "30%", paddingLeft: "2%"}} component={Stack} direction="column" justifyContent="center">
                </Paper>
            </Paper>
            <AutoCompleteDropDown value={filterJournalValue} setValue={setFitlerJournalValue} title={"Filter Publications By Journal"} DropDownArray={allJournals} />
            <AutoCompleteDropDown value={contentToSearchForValue} setValue={setContentToSearchForValue} title={"Type Of Content To Search For"} DropDownArray={["Everything", "Researchers", "Publications"]} />
            <Paper square={true} elevation={0} sx={{width: "100%", p: "2%", borderBottom: 1, borderColor: "rgba(0, 0, 0, 0.12)"}} component={Stack} direction="row">
                <Paper elevation={0} sx={{width: "20%", paddingRight: "2%"}} component={Stack} direction="column" justifyContent="center">
                </Paper>
                <Paper square={true} elevation={0} sx={{width: "50%", flexDirection: "row-reverse"}} component={Stack} direction="row">
                    <Button onClick={() => {routeChange()}}
                        sx={{borderRadius: 0, border: "2px solid Black", color: "black", backgroundColor: 'white'}}>
                        Search
                    </Button>
                </Paper>
                <Paper elevation={0} sx={{width: "30%", paddingLeft: "2%"}} component={Stack} direction="column" justifyContent="center">
