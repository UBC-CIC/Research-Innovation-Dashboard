...
import { getResearcherFull } from "../../graphql/queries";
...
export default function ChangeScopusIdPopup(props) {
  ...
  useEffect(() => {
    if (props.open === true) {
      getResearcherGeneralInformation().catch(() => {
        setScopusIdFound(false);
      });
    }
  }, [props.open]);
  ...
  const researcherInfo = () => {
    return (
      <Paper square={true} elevation={0} component={Stack} direction="row">
        <img
          alt="professor"
          style={{ width: "15%", height: "fit-content", marginRight: "4%" }}
          src={placeholderResearchPhoto}
        />
        <Paper elevation={0} sx={{ width: "72%" }}>
          <Typography>
            {"Name: " + preferred_name} <br />
          </Typography>
    ...
    <div>
      ...
      >
        ...
        <DialogContent sx={{ fontFamily: "Arial" }}>
          {!scopusIdFound && (
            <Typography>
              No researcher found with scopus ID: {props.oldScopusId}
            </Typography>
          )}
        ...
        <DialogActions>
          ...
          {scopusIdFound && (
            <Button onClick={handleClose}>Change Scopus ID</Button>
          )}
        </DialogActions>
      </Dialog>
    </div>
