...
def findMatch(firstNameToMatch, lastNameToMatch, listOfNamesToMatchAgainst):
    ...
    for firstLast in listOfNamesToMatchAgainst:
        jaroDistance = get_jaro_distance(lastNameToMatch, firstLast['lastName'])
        if(jaroDistance >= 0.95):
            lastNameMatches.append({'firstName': firstLast['firstName'], 'lastName': firstLast['lastName'], 'id': firstLast['id'], 'jaroDistance': jaroDistance})
    ...
    for firstLast in lastNameMatches:
        jaroDistance = get_jaro_distance(firstNameToMatch, firstLast['firstName'])
        if(jaroDistance >= 0.95):
...
def createResearcherList():
    ...
    response = sm_client.get_secret_value(SecretId="postgreSQL-dev-db")["SecretString"]
...
def assignIdToCleanData(bucket, key_clean1, key_clean2):
    ...
    df_clean = pd.read_csv(io.StringIO(response["Body"].read().decode("utf-8")), header=0, keep_default_na=False)
    ...
    with io.StringIO() as csv_buffer:
        ...
        response = s3_client.Object(bucket_name=bucket, key=key_clean2).put(Body=csv_buffer.getvalue())
        status = response.get("ResponseMetadata", {}).get("HTTPStatusCode")
